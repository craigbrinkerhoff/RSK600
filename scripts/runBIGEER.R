...
#Date: Fall 2020
#Description: run BEE algorithm on pepsi rivers
...
#model settings-------------------------------
g <- 9.8 #m/s2
model_uncertainity <- 0.21 #see monte_carlo_analysis.R for actual uncertainty
...
setwd('/home/cbrinkerhoff_umass_edu/RS_evasion/BIGEER/')
...
#run model-------------------------------------------------------
output <- data.frame('river'= NA, 'time'=NA, 'kobs'=NA, 'kest_mean'=NA, 'kest_low'=NA, 'kest_high'=NA)
for (currPepsi in files[1:length(files)]){
  ...
  #run BIGEER------------------------------------------
 # bedSlope <- matrix(mean(S_obs, na.rm=T), nrow=nrow(S_obs), ncol=ncol(S_obs))
  data <- BIGEER::bigee_data(w=W_obs, s=S_obs, dA=dA_obs)
  priors <- BIGEER::bigee_priors(data)
  priors$river_type_priors$lowerbound_logk600 = log(0.001) #currently setting these b/c the class-specific bounds are terrible...
  priors$river_type_priors$upperbound_logk600 = log(500)
  run_bigee <- BIGEER::bigee_estimate(bigeedata = data, bigeepriors = priors)
  kest <- BIGEER::bigee_k600pred(run_bigee)
  
  #write to file
  temp <- data.frame('river'= rep(name, length(k_obs)), 'time'=kest$time, 'kobs'=k_obs, 'kest_mean'=kest$mean, 'kest_low'=kest$conf.low, 'kest_high'=kest$conf.high)
