...
run_BIKER <- function(currPepsi, errFlag) {
  ...
  if (errFlag == 1){
    ...
    D_obs <- area/W_obs
    Rh_obs <- area / (Wtrue + 2*D_obs)
    k_obs <- k600_ustar_craig(Rh_obs, Strue) #k600 equation
    ...
    k_obs <- k600_ustar_craig(Rh_obs, S_obs) #k600 equation
    k_obs <- colMeans(k_obs, na.rm=T)
  }
