...
4. Below are boxplots of performance metrics for the 32 rivers, i.e. how well do individual rivers do? NRMSE, RRMSE, and rBIAS are optimally zero while KGE is optimally 1. A KGE greater than -0.41 indicates our model outperforms a mean estimate, akin to an NSE of 0 for the hydrologists out there (Knoben etal 2019). Also see Knoben et al (2019) for why NSE completely blows up for k600.
    a. The result is great KGEs, NRMSEs, and RRMSEs (espically for aboslutely no in situ information) however a pretty high rBIAS. This bias is borne out in the scatterplot earlier.

![**Figure 5** *Pepsi validation by river*](outputs//validation//validation_by_river.jpg)

5. Below are timeseries plots of observed versus modeled k600 for three rivers randomly sampled from the three tertiles of the by-river kge scores. a) is a 'good' river, b is a 'reasonabley performing' river, and c is a 'bad' performing river.

![**Figure 6** *Pepsi validation timeseries for three rivers, where the model results include the posterior means and 95% CIs: a) randomly selected from upper tertile of kge scores b) randomly selected from middle tertile, c) randomly selected from worse tertile*](outputs//validation//validation_timeseries.jpg)
