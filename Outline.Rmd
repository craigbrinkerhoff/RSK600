...
1) Address bias from k600 prior
2) redo fig 6 with sigma_man not kest_sd
2) Working on 3rd research question (courtesy of Merritt)
...
![**Figure 4** *Validation of remote sensing algorithm for 22 rivers with 11 day sampling intervals. Black bars are 95% CIs for the modeled values. Black line is linear regression and dashed grey line is 1:1 line.*](outputs//validation//validation.jpg)
...
6. We also compare a priori model uncertainity (i.e. from Manning's equation) against posterior model uncertainity to see how much uncertianity is added by using only observed W and H, i.e. a test of the manifestation of the priors and equifinality.

THIS NEEDS TO BE REDONE WITH SIGMA_MAN

![**Figure 6** *A) Histogram of BIKER k600 uncertainities (across all rivers and timesteps) compared against a priori uncertainity from Manning's equation (0.25- dark blue line). Red line is the mean of the BIKER uncertainities (0.32). B) The same plot, but mean uncertaintities for the 22 rivers (red line at 0.33).*](outputs//validation//validation_uncertainity.jpg)

I'm still trying to figure out why there's like five rivers that have crazy increased uncertianity (causing the bimodal distributions).
...
![**Figure 7** *Left: FCO2 from BIKER versus using observed average flow velocity for all timesteps for 22 SWOT rivers. right: timeseries plots for three example rivers.*](outputs//flux_implications//FCO2_plot.jpg)
