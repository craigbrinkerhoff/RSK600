...
fun_FCO2_analysis <- function(river) {
  ...
  #avgerage of the models was the actual implemntation in Raymond 2013
  ...
  k600_Lauerwald2015 <- k600_craig(W_obs, S_obs, V_lauerwald2015)
  ...
  k600_Lauerwald2015_low <- k600_craig(W_obs, S_obs, V_lauerwald2015_low)
  k600_Lauerwald2015_high <- k600_craig(W_obs, S_obs, V_lauerwald2015_high)
  ...
  for_plot <- data.frame('river' = name, 'timestep'=1:length(FCO2_BIKER), 'sa'=sa,
    ...
    write.csv(for_plot, paste0('outputs/FCO2/by_river/results_', name, '.csv'))
}

s <- data.frame('river'=NA, 'meanS'=NA)
#run-------------------------------------------
#read in co2 data (Beauliu etal 2012)--------------------------------
data <- read.csv('inputs/FCO2/Beauliu2012.csv', header = TRUE)
...
df_fin <- list.files(path='outputs/FCO2/by_river', full.names = TRUE) %>%
  lapply(read_csv) %>%
  bind_rows
write.csv(df_fin, 'outputs/FCO2/CO2_results.csv')
...
ggsave('outputs/FCO2/Beaulieu_timeseries.jpg', beaulieu, width=15, height=6)

#Summary stats-------------------------------
output <- read.csv('outputs/FCO2/CO2_results.csv')
...
write.csv(massFluxes, 'outputs//FCO2//bulkFluxes.csv')
...
ggsave('outputs/FCO2/FCO2_models.jpg', FCO2_models, width=12, height=6)
...
rmse <- round((Metrics::rmse(log10(output$FCO2_ulset), log10(output$FCO2_BIKER))), 2) #g/m2/dy
...
flux_plot<- ggplot(output, aes(x=(FCO2_ulset), y=(FCO2_BIKER), color=key)) +
  ...
  theme(legend.position = "none",
        ...
        legend.text = element_text(size=17),
        legend.title = element_text(size=17, face='bold')) +
  geom_richtext(aes(x=10^-0.4, y=10^1), color='black', label=paste0('RMSE: 10^', rmse, ' g/m2*dy'), size=6) +
  geom_richtext(aes(x=10^-0.5, y=10^0.8), color='black', label=paste0('r<sup>2</sup>: ', lmr2), size=6)
...
write.csv(results_all_rivs, 'outputs//FCO2//fco2_stats_all.csv')
write.csv(stats_by_reach, 'outputs//FCO2//fco2_stats_by_river.csv')
