...
#function for parallelzing BIKER runs with baked in SWOT slope andheight errors------------------------------
run_BIKER_err <- function(currPepsi) {
#run model-------------------------------------------------------
  ...
  write.csv(temp, paste0('outputs/temp/results_err_', name, '.csv'))
}
...
run_BIKER <- function(currPepsi, errFlag) {
  ...
  temp <- data.frame('river'= rep(name, length(k_obs)), 'time'=kest$time, 'kobs'=k_obs, 'kest_mean'=kest$mean, 'kest_low'=kest$conf.low, 'kest_high'=kest$conf.high, 'kest_sd'=kest$sigma)
  if(errFlag == 1){
    write.csv(temp, paste0('outputs/temp/results_', name, '_err.csv'))
  }
  else{
    write.csv(temp, paste0('outputs/temp/results_', name, '.csv'))
  }
}
